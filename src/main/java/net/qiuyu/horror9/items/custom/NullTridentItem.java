...
public class NullTridentItem extends TridentItem {
    ...
    public void releaseUsing(ItemStack pStack, Level pLevel, LivingEntity pLivingEntity, int pTimeLeft) {
        if (pLivingEntity instanceof Player player) {
            ...
            if (i >= 10) {
                int j = (int) EnchantmentHelper.getTridentSpinAttackStrength(pStack, player);
                if (j <= 0 || player.isInWaterOrRain()) {
                    if (!pLevel.isClientSide) {
                        ...
                        if (j == 0) {
                            ...
                            pLevel.playSound((Player)null, nulltrident, SoundEvents.TRIDENT_THROW.value(), SoundSource.PLAYERS, 1.0F, 1.0F);
                            if (!player.getAbilities().instabuild) {
